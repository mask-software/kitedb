...
use crate::constants::{
    DEFAULT_PAGE_SIZE, LOCK_BYTE_OFFSET, LOCK_BYTE_RANGE, MAX_PAGE_SIZE, MIN_PAGE_SIZE,
    OS_PAGE_SIZE,
};
...
impl FilePager {
    ...
    pub fn allocate_pages(&mut self, count: u32) -> Result<u32> {
        if count == 0 {
            return Err(RayError::Internal("Must allocate at least 1 page".to_string()));
        }
