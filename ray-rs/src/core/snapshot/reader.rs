...
impl SnapshotData {
    ...
    pub fn parse_at_offset(mmap: Arc<Mmap>, offset: usize, options: &ParseSnapshotOptions) -> Result<Self> {
    ...
    pub fn section_bytes(&self, id: SectionId) -> Option<Vec<u8>> {
        ...
        let compression = CompressionType::from_u32(section.compression)
        ...
        // Decompress
        let decompressed = decompress_with_size(
            raw_bytes,
            compression,
    ...
    pub fn find_edge_index(&self, src_phys: PhysNode, etype: ETypeId, dst_phys: PhysNode) -> Option<usize> {
    ...
    fn binary_search_key_hash(&self, entries: &[u8], hash64: u64, num_entries: usize) -> (usize, usize) {
        let mut lo = 0;
...
impl<'a> Iterator for InEdgeIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let out_index = self.in_out_index
            .and_then(|idx| {
