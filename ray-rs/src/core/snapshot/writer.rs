    ...
    for src_phys in 0..num_nodes {
        ...
        for out_idx in start..end {
            ...
            in_edges_by_node
                .entry(dst_phys)
                .or_default()
                .push((src_phys as PhysNode, edge_etype, out_idx as u32));
        }
    }
...
pub fn build_snapshot(db_path: &Path, input: SnapshotBuildInput) -> Result<String> {
    ...
    let has_properties = nodes.iter().any(|n| !n.props.is_empty())
        || edges.iter().any(|e| !e.props.is_empty());
    ...
    for id_num in 0..SectionId::COUNT {
        ...
        let (sec_offset, sec_length, compression, uncompressed_size) =
            section_offsets.get(&id).copied().unwrap_or((0, 0, CompressionType::None, 0));
...
mod tests {
    ...
    fn test_csr_building() {
        let nodes = vec![
            NodeData { node_id: 1, key: None, labels: vec![], props: HashMap::new() },
            NodeData { node_id: 2, key: None, labels: vec![], props: HashMap::new() },
            NodeData { node_id: 3, key: None, labels: vec![], props: HashMap::new() },
        ];

        let edges = vec![
            EdgeData { src: 1, etype: 1, dst: 2, props: HashMap::new() },
            EdgeData { src: 1, etype: 1, dst: 3, props: HashMap::new() },
            EdgeData { src: 2, etype: 2, dst: 1, props: HashMap::new() },
        ];
