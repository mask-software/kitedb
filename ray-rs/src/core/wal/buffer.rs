...
impl WalBuffer {
    ...
    pub fn free(&self) -> u64 {
        if self.active_region == 0 {
            ...
            self.primary_region_size.saturating_sub(self.primary_head + 1)
        } else {
            // Secondary region available space
            self.secondary_region_size.saturating_sub(self.secondary_head - self.secondary_region_start + 1)
        }
    }
    ...
    pub fn scan_region(&mut self, region: u8, pager: &mut FilePager) -> Result<Vec<ParsedWalRecord>> {
        ...
        let (mut pos, end_pos, region_start) = if region == 0 {
            ...
            (self.secondary_region_start, self.secondary_head, self.secondary_region_start)
        };
    ...
    fn write_record_bytes_to_primary(&mut self, record_bytes: &[u8], pager: &mut FilePager) -> Result<u64> {
    ...
    pub fn get_records_for_recovery(&mut self, pager: &mut FilePager) -> Result<Vec<ParsedWalRecord>> {
