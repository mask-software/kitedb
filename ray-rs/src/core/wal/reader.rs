...
pub fn validate_wal<P: AsRef<Path>>(filepath: P) -> Result<WalValidationResult> {
    ...
    let last_record_end = records.last().map(|r| r.record_end).unwrap_or(WAL_HEADER_SIZE);
    let has_trailing_data = last_record_end < buffer.len() && buffer[last_record_end..].iter().any(|&b| b != 0);
