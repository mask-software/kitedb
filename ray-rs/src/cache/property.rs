...
impl PropertyCache {
    ...
    pub fn peek_node_prop(&self, node_id: NodeId, prop_key_id: PropKeyId) -> Option<&Option<PropValue>> {
        let key = (node_id, prop_key_id);
...
mod tests {
    ...
    fn test_all_prop_value_types() {
        ...
        assert_eq!(cache.get_node_prop(1, 2), Some(&Some(PropValue::Bool(true))));
        assert_eq!(cache.get_node_prop(1, 3), Some(&Some(PropValue::I64(-123))));
        assert_eq!(cache.get_node_prop(1, 4), Some(&Some(PropValue::F64(3.14159))));
        assert_eq!(cache.get_node_prop(1, 5), Some(&Some(PropValue::String("hello world".to_string()))));
        assert_eq!(cache.get_node_prop(1, 6), Some(&Some(PropValue::VectorF32(vec![1.0, 2.0, 3.0]))));
    }
    ...
    fn test_invalidate_nonexistent_edge() {
        ...
        assert_eq!(cache.get_edge_prop(1, 1, 2, 10), Some(&Some(PropValue::I64(42))));
    }
}
