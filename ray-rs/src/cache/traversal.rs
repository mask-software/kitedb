...
use crate::types::{Edge, ETypeId, NodeId, TraversalCacheConfig};
...
/// # Example
/// ```
/// use raydb_core::cache::traversal::{TraversalCache, TraversalDirection};
/// use raydb_core::types::{TraversalCacheConfig, Edge};
///
...
impl TraversalCache {
    ...
    pub fn set(
        ...
        let (cached_neighbors, truncated) = if neighbors.len() > self.max_neighbors_per_entry {
            (
                neighbors.into_iter().take(self.max_neighbors_per_entry).collect(),
                true,
            )
        } else {
            (neighbors, false)
        ...
        self.cache.set(key, CachedNeighbors {
...
mod tests {
    ...
    fn test_invalidate_node() {
        ...
        cache.set(1, Some(1), TraversalDirection::Out, vec![make_edge(1, 1, 2)]);
        cache.set(2, Some(1), TraversalDirection::Out, vec![make_edge(2, 1, 3)]);
    ...
    fn test_invalidate_edge() {
        ...
        cache.set(3, Some(1), TraversalDirection::Out, vec![make_edge(3, 1, 4)]);
    ...
    fn test_invalidate_edge_all_etypes() {
        ...
        cache.set(1, None, TraversalDirection::Out, vec![
            make_edge(1, 1, 2),
            make_edge(1, 2, 3),
        ]);
    ...
    fn test_peek_does_not_update_lru() {
        ...
        cache.set(1, Some(1), TraversalDirection::Out, vec![make_edge(1, 1, 10)]);
        cache.set(2, Some(1), TraversalDirection::Out, vec![make_edge(2, 1, 20)]);
        ...
        cache.set(3, Some(1), TraversalDirection::Out, vec![make_edge(3, 1, 30)]);
    ...
    fn test_key_uniqueness() {
        ...
        // Different node IDs
        cache.set(1, Some(1), TraversalDirection::Out, vec![make_edge(1, 1, 100)]);
        cache.set(2, Some(1), TraversalDirection::Out, vec![make_edge(2, 1, 200)]);
        
        // Different etypes
        cache.set(1, Some(2), TraversalDirection::Out, vec![make_edge(1, 2, 300)]);
        
        // Different directions
        cache.set(1, Some(1), TraversalDirection::In, vec![make_edge(100, 1, 1)]);
        
        assert_eq!(cache.len(), 4);
        
        // Each should be retrievable independently
        assert_eq!(cache.peek(1, Some(1), TraversalDirection::Out).unwrap().neighbors[0].dst, 100);
        assert_eq!(cache.peek(2, Some(1), TraversalDirection::Out).unwrap().neighbors[0].dst, 200);
        assert_eq!(cache.peek(1, Some(2), TraversalDirection::Out).unwrap().neighbors[0].dst, 300);
        assert_eq!(cache.peek(1, Some(1), TraversalDirection::In).unwrap().neighbors[0].src, 100);
    }
