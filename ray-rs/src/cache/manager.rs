...
use crate::types::{
    CacheOptions, CacheStats, Edge, ETypeId, NodeId, PropKeyId, PropValue,
    PropertyCacheConfig, QueryCacheConfig, TraversalCacheConfig,
};
...
impl CacheManager {
    ...
    pub fn get_node_prop(&mut self, node_id: NodeId, prop_key_id: PropKeyId) -> Option<&Option<PropValue>> {
        if !self.enabled {
            return None;
        }
    ...
    pub fn set_node_prop(&mut self, node_id: NodeId, prop_key_id: PropKeyId, value: Option<PropValue>) {
        if !self.enabled {
            return;
        }
        self.property_cache.set_node_prop(node_id, prop_key_id, value);
    }
    ...
    pub fn get_edge_prop(
        ...
        self.property_cache.get_edge_prop(src, etype, dst, prop_key_id)
    }
    ...
    pub fn set_edge_prop(
        ...
        self.property_cache.set_edge_prop(src, etype, dst, prop_key_id, value);
    }
    ...
    pub fn set_traversal(
        ...
        self.traversal_cache.set(node_id, etype, direction, neighbors);
    }
...
mod tests {
    ...
    fn test_disabled_noop() {
        ...
        assert!(cache.get_traversal(1, Some(1), TraversalDirection::Out).is_none());
    ...
    fn test_invalidate_edge() {
        ...
        cache.set_traversal(2, Some(1), TraversalDirection::In, vec![
            Edge { src: 1, etype: 1, dst: 2 },
        ]);
        ...
        assert_eq!(cache.get_edge_prop(1, 2, 3, 10), Some(&Some(PropValue::I64(100))));
    }
    ...
    fn test_clear_individual_caches() {
        ...
        assert_eq!(cache.get_node_prop(1, 10), None);
        assert!(cache.get_traversal(1, Some(1), TraversalDirection::Out).is_some());
