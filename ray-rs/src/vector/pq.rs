...
impl PqIndex {
    ...
    pub fn encode(&mut self, vectors: &[f32], num_vectors: usize) -> Result<(), PqError> {
        ...
        for i in 0..num_vectors {
            ...
            for m in 0..self.config.num_subspaces {
                let sub_offset = m * self.subspace_dims;
                let subvec = &vectors[vec_offset + sub_offset..vec_offset + sub_offset + self.subspace_dims];
...
fn train_subspace(
    ...
    initialize_centroids_kmeans_pp(centroids, subvectors, num_vectors, subspace_dims, num_centroids);
