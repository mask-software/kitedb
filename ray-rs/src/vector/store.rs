...
pub fn vector_store_insert(
    ...
    // Get fragment index
    let fragment_idx = manifest.fragments.iter().position(|f| f.id == fragment_id).unwrap();
    
    // Check if we need a new row group
    let row_group_idx = {
        let fragment = &mut manifest.fragments[fragment_idx];
        if fragment.row_groups.is_empty()
            || fragment.row_groups.last().unwrap().is_full(row_group_size)
        {
            let rg_id = fragment.row_groups.len();
            fragment.row_groups.push(RowGroup::new(rg_id, row_group_size, dimensions));
            rg_id
        } else {
...
fn vector_store_delete_by_vector_id(manifest: &mut VectorManifest, vector_id: u64) -> bool {
    ...
    let fragment = match manifest.fragments.iter_mut().find(|f| f.id == location.fragment_id) {
        Some(f) => f,
        None => return false,
    };
...
pub fn vector_store_get_by_id<'a>(
    ...
    let fragment = manifest.fragments.iter().find(|f| f.id == location.fragment_id)?;
...
pub fn vector_store_has(manifest: &VectorManifest, node_id: NodeId) -> bool {
    ...
    let fragment = match manifest.fragments.iter().find(|f| f.id == location.fragment_id) {
        Some(f) => f,
        None => return false,
    };
...
fn validate_vector(vector: &[f32]) -> Result<(), VectorStoreError> {
    ...
    for &val in vector {
        if val.is_nan() {
            return Err(VectorStoreError::InvalidVector("Vector contains NaN".into()));
        }
        if val.is_infinite() {
    ...
    if all_zero {
        return Err(VectorStoreError::InvalidVector("Vector is all zeros".into()));
    }
