...
use crate::vector::types::{Fragment, FragmentState, RowGroup, VectorLocation, VectorManifest, VectorStoreConfig};
...
pub fn compact_fragments(
    ...
    for &fragment_id in fragment_ids {
        ...
        for local_idx in 0..fragment.total_vectors {
            ...
            updated_locations.insert(vector_id, VectorLocation {
...
fn append_to_fragment(fragment: &mut Fragment, vector: &[f32], config: &VectorStoreConfig) -> usize {
    let dimensions = config.dimensions;
    let row_group_size = config.row_group_size;
...
mod tests {
    use super::*;
    use crate::vector::{create_vector_store, vector_store_insert, vector_store_delete, vector_store_seal_active};
