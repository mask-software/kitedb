    ...
    F: FnMut(NodeId, HashMap<PropKeyId, Option<PropValue>>) -> R,
{
    ...
    pub fn execute(self) -> R {
        let node_id = self.node_id.expect("Update requires a node ID (use where_id())");
    ...
    F: FnMut(NodeId) -> R,
{
    ...
    /// Execute the delete
    pub fn execute(self) -> R {
        let node_id = self.node_id.expect("Delete requires a node ID (use where_id())");
    ...
    F: FnMut(NodeId, ETypeId, NodeId) -> R,
{
    ...
    /// Execute the unlink
    pub fn execute(self) -> R {
        let dst = self.dst.expect("Unlink requires a destination (use from_node())");
...
impl BatchBuilder {
    ...
    pub fn link_with_props(
        ...
        self.ops.push(BatchOp::Link { src, etype, dst, props });
        self
    }
...
mod tests {
    ...
    fn test_node_ref_with_props() {
        ...
        assert_eq!(node.get_prop("name"), Some(&PropValue::String("Alice".to_string())));
