...
use crate::vector::{
    ...
    vector_store_stats, vector_store_clear, DistanceMetric, IvfIndex, IvfConfig, VectorManifest,
    VectorSearchResult, VectorStoreConfig, SearchOptions,
};
...
impl VectorIndex {
    ...
    fn build_index_internal(&mut self) -> Result<(), VectorIndexError> {
        ...
        index.add_training_vectors(&training_data, vector_ids.len())
    ...
    pub fn search(&mut self, query: &[f32], options: SimilarOptions) -> Result<Vec<VectorSearchHit>, VectorIndexError> {
        let dimensions = self.options.dimensions;
    ...
    fn brute_force_search(&self, query: &[f32], k: usize) -> Vec<VectorSearchResult> {
        use crate::vector::{cosine_distance, euclidean_distance, dot_product, normalize};
...
mod tests {
    ...
    fn test_similar_options() {
        let opts = SimilarOptions::new(10)
            .with_threshold(0.8)
            .with_n_probe(5);
    ...
    fn test_vector_index_dimension_mismatch() {
        ...
        assert!(matches!(result, Err(VectorIndexError::DimensionMismatch { expected: 4, got: 3 })));
    }
    ...
    fn test_brute_force_search() {
        let mut index = VectorIndex::new(
            VectorIndexOptions::new(4)
                .with_training_threshold(1000) // High threshold to force brute force
        );
    ...
    fn test_search_with_threshold() {
        ...
        let results = index.search(
            &query, 
            SimilarOptions::new(10).with_threshold(0.5)
        ).unwrap();
