...
use crate::types::{
    EdgeVersionData, ETypeId, NodeDelta, NodeId, NodeVersionData, PropKeyId, PropValue, Timestamp,
    TxId,
};
...
impl VersionChainManager {
    ...
    pub fn get_node_prop_version(
        ...
        if self.use_soa {
            self.soa_node_props.get_head(key).map(|pooled| {
                self.pooled_to_versioned(&self.soa_node_props, pooled)
            })
        } else {
    ...
    pub fn get_edge_prop_version(
        ...
        if self.use_soa {
            self.soa_edge_props.get_head(key).map(|pooled| {
                self.pooled_to_versioned(&self.soa_edge_props, pooled)
            })
        } else {
            self.legacy_edge_props.get(&key).map(|b| {
                self.clone_versioned_record(b.as_ref())
            })
        }
    }
    ...
    fn pooled_to_versioned(
        ...
        let prev = if pooled.prev_idx != NULL_IDX {
            store.get_at(pooled.prev_idx).map(|prev_pooled| {
                Box::new(self.pooled_to_versioned(store, prev_pooled))
            })
        } else {
            None
        };
    ...
    fn clone_versioned_record(
        ...
        VersionedRecord {
            ...
            prev: record.prev.as_ref().map(|p| Box::new(self.clone_versioned_record(p))),
    ...
    pub fn truncate_deep_chains(&mut self, max_depth: usize, min_active_ts: Option<Timestamp>) -> usize {
        ...
        if self.use_soa {
            truncated += self.soa_node_props.truncate_deep_chains(max_depth, min_active_ts);
            truncated += self.soa_edge_props.truncate_deep_chains(max_depth, min_active_ts);
        } else {
