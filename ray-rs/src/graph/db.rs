...
pub fn open_graph_db<P: AsRef<Path>>(path: P, options: OpenOptions) -> Result<GraphDB> {
    ...
    let (mut next_node_id, mut next_label_id, mut next_etype_id, mut next_propkey_id) = if let Some(ref snap) = snapshot {
        (
    ...
    if let Ok(Some(wal_segment)) = load_wal_segment_by_id(path, manifest.active_wal_seg) {
        ...
        use crate::core::wal::record::{extract_committed_transactions,
            parse_create_node_payload, parse_delete_node_payload, parse_add_edge_payload,
            parse_delete_edge_payload, parse_set_node_prop_payload, parse_del_node_prop_payload,
            parse_define_label_payload, parse_define_etype_payload, parse_define_propkey_payload};
...
mod tests {
    ...
    fn test_wal_replay_on_reopen() {
        ...
        // Second session: verify data was recovered from WAL
        {
            ...
            assert!(delta.is_edge_added(node_ids[0], 1, node_ids[1]), "Edge should be in delta after WAL replay");
    ...
    fn test_wal_replay_definitions() {
        use crate::graph::tx::{begin_tx, commit};
        use crate::graph::definitions::{define_label, define_etype, define_propkey};
    ...
    fn test_wal_replay_node_properties() {
        use crate::graph::tx::{begin_tx, begin_read_tx, commit};
