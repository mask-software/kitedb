/**
 * Snapshot writer - builds CSR snapshots from nodes and edges
 */
...
export async function buildSnapshot(
  ...
  const inCSR = buildInEdgesCSR(nodes, outCSR, nodeIdToPhys, physToNodeId);
  ...
  // Note: nodes are already sorted by nodeId at this point
  for (const node of nodes) {
    for (const [keyId, value] of node.props) {
  ...
  for (const edge of edges) {
    for (const [keyId, value] of edge.props) {
      if (value.tag === PropValueTag.STRING) {
        internString(stringTable, value.value);
      }
    }
  ...
  {
    // Build node property data in CSR format
    // Nodes are already sorted by nodeId, so we iterate in order
  ...
  {
    // Edges are sorted within each node by (etype, dst) in buildOutEdgesCSR
    // We need to match the same order for edge properties
    // Since edges input might be in different order, we need to reorder
